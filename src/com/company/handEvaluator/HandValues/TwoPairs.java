...
/**
*  Two Pairs -
 * The com.company.hand contains 2 different pairs.
 ...
 * If these values are the same the hands are ranked by the value of the remaining com.company.card.
 * **/
public class TwoPairs implements Evaluator {
    private Hand hand1;
    private Hand hand2;
    ...
    public Hand evaluate(){
        ...
        int h2Pair2 = 0;
        for(int i=1; i<5; i++) {
            Card hand1i = hand1.getHand().get(i);
            Card hand1iMinusOne = hand1.getHand().get(i-1);
            Card hand2i = hand2.getHand().get(i);
            Card hand2iMinusOne = hand2.getHand().get(i-1);
        ...
        if((h1Pair1Equalh2Pair1 || h1Pair2Equalh1Pair1) && (h1Pair1Equalh2Pair2 || h1Pair2Equalh2Pair2) && (h1Pair1 > 0) && (h2Pair1 > 0) && (h1Pair2 > 0) && (h2Pair2 > 0)) {
            ...
            //lone cards ( 1 1 2 2 A ) -> lone com.company.card would be A
            int h1 = 0;
            int h2 = 0;
            //find lone com.company.card
            for(int i=1; i<5; i++) {
                Card hand1i = hand1.getHand().get(i);
                Card hand2i = hand2.getHand().get(i);
        ...
        //One pair is same for both com.company.hand so we rank by the other pair that isn't the same with both.
